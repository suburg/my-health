...
- [x] T034 [P] [US4] Создать `VisitCard.tsx` — 3 блока, скан, рейтинг, «Не указано»
- [x] T035 [US4] Ссылки «Предыдущий/Следующий приём» через `findPrevVisit`/`findNextVisit`
- [x] T036 [US4] Клик по плитке → открытие карточки (через `VisitDetailView`)
- [x] T037 [US4] Создать `VisitDetailView.tsx` — контейнер с кнопками редактирования/удаления
