...
- [x] T035 [P] Добавить логирование IPC-вызовов health-модуля при debug: true (logger.ts + обёртка в health-service)
- [x] T036 [P] Написать unit-тесты для health_store.rs (запись, чтение, удаление, атомарность)
- [x] T037 [P] Написать unit-тесты для zod-схем валидации health-данных и deviation-utils
- [x] T038 [P] Проверка граничных случаев: пустая таблица (0 замеров), один замер, 100+ замеров, некорректный формат давления/сна, нормализация формата «6ч30м» → минуты
- [x] T039 Финальное тестирование полного потока: новый замер → заполнение → редактирование → удаление → графики → фильтрация периода
